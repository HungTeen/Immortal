    ...
    public static void spawnLineSpiritualParticle(Level world, Triple<Float, Float, Float> color, Vec3 origin, Vec3 target, float particleRatio, int particleCountEach, double offsetScale) {
        ...
        for(int i = 0; i < particleNum; ++i) {
            for(int j = 0; j < particleCountEach; ++j) {
                Vec3 pos = origin.add(target.subtract(origin).normalize().scale(Math.max(1.0D, distance - 2.0D) / (double)particleNum * (double)(i + 1) / (double)particleRatio)).add(MathUtil.getRandomVec3(world.getRandom(), offsetScale));
                hungteen.htlib.util.ParticleUtil.spawnParticles(world, ImmortalParticles.SPIRITUAL_RELEASING.get(), pos, color.getLeft(), color.getMid(), color.getRight());
            }
        }
