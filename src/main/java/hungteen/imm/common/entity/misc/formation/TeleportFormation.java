...
public class TeleportFormation extends FormationEntity {
    ...
    public void tick() {
        ...
        if (EntityHelper.isServer(this) && level() instanceof ServerLevel serverLevel) {
            ...
            ParticleHelper.spawnParticles(this.level(), IMMParticles.SPIRITUAL_MANA.get(), getX(), getY(), getZ(), 5, TELEPORT_WIDTH, TELEPORT_HEIGHT, 0.2F, 0.1F);
        }
    }

    private void spawnTeleportParticles(Entity entity, int ticks) {
        final int cnt = ticks / 8 + 1;
        ParticleHelper.spawnParticles(this.level(), IMMParticles.SPIRITUAL_MANA.get(), getX(), getY(), getZ(), cnt, 0.5, 0.25, 0.2F, 0.1F);
    }
